    ...
    var body: some View {
        ZStack {
            ...
            if snapshot.isBabySleeping {
                ...
                // No counter here — the Live Activity owns the live timer.
                VStack(spacing: 0) {
                    HStack(spacing: 2) {
...
private struct RectangularLockView: View {
    ...
    // MARK: Sleeping — display only (Live Activity owns interactive controls now)
    ...
    // MARK: Active feed — display only. Controls live on the Live Activity banner.
    ...
    @ViewBuilder private var activeFeedLayout: some View {
        VStack(alignment: .leading, spacing: 1) {
            ...
            // No counter — the Live Activity banner shows the live timer.
            Text("\(FeedFormat.sideLetter(from: snapshot.activeSessionSide)) breast")
...
private struct InlineLockView: View {
    ...
    var body: some View {
        if snapshot.isBabySleeping {
            ...
            // No counter — Live Activity shows the live timer.
            let status = snapshot.isActivePaused ? "Paused" : "Feeding"
            Text("Nourish · \(status) \(FeedFormat.sideLetter(from: snapshot.activeSessionSide))")
        } else if snapshot.lastFeedTime != nil {
